...
impl Script {
    ...
    pub fn compile(
        ...
        let raw = unsafe {
            r#try!(util::invoke_ctx(isolate, context, |c| {
                v8::v8_Script_Compile(c, context.as_raw(), source.as_raw())
            }))
        };
    ...
    pub fn run(&self, context: &context::Context) -> error::Result<value::Value> {
        unsafe {
            let raw = r#try!(util::invoke_ctx(&self.0, context, |c| v8::v8_Script_Run(
                c,
                self.1,
                context.as_raw()
            )));
